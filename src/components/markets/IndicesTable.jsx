...
const IndicesTableRow = memo(({ market, onMarketClick, formatPrice, isFavorite, isWatchlist, onToggleFavorite, onToggleWatchlist }) => {
  const name = market.id || market.symbol;
  const type = market.marketType || 'indices';
  ...
  return (
    <tr className="indicesTableRow" onClick={handleRowClick}>
      <td className="indicesTableCell marketCell">
...
IndicesTableRow.displayName = 'IndicesTableRow';

const IndicesMobileCard = memo(({ market, onMarketClick, formatPrice, isFavorite, isWatchlist, onToggleFavorite, onToggleWatchlist }) => {
  const name = market.id || market.symbol;
  const type = market.marketType || 'indices';
  ...
  return (
    <div className="indicesMobileCard" onClick={handleCardClick}>
      <div className="indicesCardHeader">
        <div className="indicesCardPair">
          <button
      ...
      <div className="indicesCardBody">
        <div className="indicesCardRow">
          <div className="indicesCardLabel">Price</div>
          <div className="indicesCardValue">{formatPrice(market.price || market.index)}</div>
        </div>
        <div className="indicesCardRow">
          <div className="indicesCardLabel">Spread</div>
          <div className="indicesCardValue">{formatPrice((market.ask || 0) - (market.bid || 0))}</div>
        </div>
        <div className="indicesCardRow">
          <div className="indicesCardLabel">24h Change</div>
          <div className={`indicesCardValue ${market.change24h >= 0 ? 'positive' : 'negative'}`}>
            {market.change24h >= 0 ? '+' : ''}{market.change24h.toFixed(2)}%
          </div>
        </div>
        <div className="indicesCardRow">
          <div className="indicesCardLabel">Ask</div>
          <div className="indicesCardValue askValue">{formatPrice(market.ask)}</div>
        </div>
        <div className="indicesCardRow">
          <div className="indicesCardLabel">Bid</div>
          <div className="indicesCardValue bidValue">{formatPrice(market.bid)}</div>
        </div>
      </div>
    </div>
  );
...
IndicesMobileCard.displayName = 'IndicesMobileCard';

const IndicesTable = ({ markets, onMarketClick, formatPrice, favoritesSet, watchlistSet, itemKey, onToggleFavorite, onToggleWatchlist }) => {
  const keyOf = (m) => itemKey(m.id || m.symbol, m.marketType || 'indices');
  return (
    <>
      <div className="indicesTableWrapper">
        <table className="indicesTable">
          <thead className="indicesTableHead">
            <tr>
              <th className="indicesTableHeader marketCell">Index</th>
              <th className="indicesTableHeader priceCell">Last Price</th>
              <th className="indicesTableHeader changeCell">24h Change</th>
              <th className="indicesTableHeader askCell">Bid</th>
              <th className="indicesTableHeader bidCell">Ask</th>
              <th className="indicesTableHeader spreadCell">Spread</th>
              <th className="indicesTableHeader highCell">24h High</th>
              <th className="indicesTableHeader lowCell">24h Low</th>
              <th className="indicesTableHeader tradeCell">Trade</th>
            </tr>
          </thead>
          <tbody className="indicesTableBody">
            {markets.length > 0 ? (
              markets.map((market) => (
                <IndicesTableRow
              ...
              <tr>
                <td colSpan="9" className="indicesTableEmpty">
                  <div className="noMarkets">
                    <p>No indices markets available</p>
                  </div>
                </td>
              </tr>
            )}
      ...
      <div className="indicesMobileView">
        {markets.length > 0 ? (
          markets.map((market) => (
            <IndicesMobileCard
              key={market.id || market.symbol}
