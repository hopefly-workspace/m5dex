...
## Flutter WebView — standalone crypto chart URL

Use a **dedicated SPA route** (no dashboard chrome) and pass the pair from Flutter.

| Route | Example |
|--------|---------|
| Query | `{origin}/webview/crypto-chart?pair=BTCUSDT` |
| Path | `{origin}/webview/crypto-chart/BTCUSDT` |

- Query `pair` or `symbol` overrides the path segment when both are present.
- Optional `interval` must match chart timeframes (e.g. `15m`, `1h`; default `15m`).
- Login is **not** required for candles. Platform TP/SL lines appear only when the WebView session has auth (same storage/cookies as the web app).

**Flutter:**

```dart
final base = Uri.parse('https://your-deployed-origin');
final uri = base.replace(path: '${base.path}webview/crypto-chart', queryParameters: {'pair': 'ETHUSDT', 'interval': '15m'});
